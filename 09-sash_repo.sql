...
spool sash_repo.log
prompt Creating SASH_REPO package
...
CREATE OR REPLACE PACKAGE sash_repo AS
    ...
    procedure add_instance_job (v_dbname varchar2, v_inst_num number);
...
FUNCTION get_dbid return number is
    ...
    begin
      ...
      execute immediate 'select dbid  from sash_targets where rownum<2' into l_dbid;
      return l_dbid;
end get_dbid;
...
-- delete rows from not ASH tables
    ...
    -- delete from SASH_SQLSTATS where dbid, instance_snap_id in (select HIST_SAMPLE_ID from SASH_HIST_SAMPLE where HIST_DATE < sysdate - l_minusdays);
...
begin
    ...
    for i in (select inst_num, dbname from sash_targets) loop
        
        add_instance_job( i.dbname, i.inst_num); 
        /*                  
	...
	v_dbid := get_dbid;
    vwhat:='begin sash_pkg.get_all; end;';
    dbms_scheduler.create_job(job_name => 'sash_pkg_get_all_' || v_dbid,
                          job_type=>'PLSQL_BLOCK',
                          job_action=> vwhat,
                          -- start_date=>to_date(mod(trunc((to_char(sysdate,'SSSSS')+v_startmin)/v_startmin)*v_startmin,86399),'SSSSS'),
                          start_date=>(trunc(sysdate) + (mod(trunc((to_char(sysdate,'SSSSS')+v_startmin)/v_startmin)*v_startmin,86399))/86400),
                          repeat_interval=>'FREQ = MINUTELY; INTERVAL = ' || v_getall,
                          enabled=>true);
	-- dbms_scheduler.set_attribute('sash_pkg_get_all_' || v_inst_num || '_' || v_dbid, 'INSTANCE_ID', v_inst_num);

    log_message('add_instance_job','adding scheduler job sash_pkg_get_all_' || v_dbid,'I');
...
begin
    execute immediate 'select version from sys.v_$instance' into s_version;
    if (substr(s_version,1,2) > '11') then
    	execute immediate 'select con_dbid from sys.v_$database' into v_dbid;
    else	
    	execute immediate 'select dbid from sys.v_$database' into v_dbid;
    end if;
    select count(*) into v_check from sash_targets where dbid = v_dbid and inst_num = v_inst_num;
    if v_check = 0 then 
        insert into sash_targets (dbid, host, port, dbname, sid, inst_num, version, cpu_count) values (v_dbid, v_host, v_port, v_db_name, v_sid, v_inst_num, coalesce(v_version, s_version), v_cpu_count);
    else 
...
procedure add_instance_job (v_dbname varchar2, v_inst_num number)  is
vwhat varchar2(4000);
v_getall number;
v_startmin number;
v_dbid number;
...
begin
	...
	vwhat:='begin sash_pkg.collect_ash(10,360,' || v_inst_num || '); end;';
    dbms_scheduler.create_job(job_name => 'sash_pkg_collect_' || v_inst_num || '_' || v_dbid,
                            ...
                            repeat_interval => 'FREQ = HOURLY; INTERVAL = 1',
                            enabled=>true);
	dbms_scheduler.set_attribute('sash_pkg_collect_' || v_inst_num || '_' || v_dbid, 'INSTANCE_ID', v_inst_num);
