...
create or replace view dba_hist_snapshot (SNAP_ID,DBID,INSTANCE_NUMBER,BEGIN_INTERVAL_TIME, END_INTERVAL_TIME, STARTUP_TIME) as select hist_sample_id, dbid, instance_number, last_date, hist_date, startup_time from SASH_HIST_SAMPLE;
...
CLIENT_ID                
	 ...
	 where sample_time between BEGIN_INTERVAL_TIME and BEGIN_INTERVAL_TIME + (select nvl(max(value),15) from sash_configuration where param='STATFREQ')/24/60;
